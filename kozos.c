...
{
  puts_spike(current->name);
  puts_spike(" DOWN. mcause: "); putxval(mcause, 0);
  puts_spike(" mepc: "); putxval(mepc, 0); puts_spike("\n");
...
void handle_sync_trap(uint32_t *sp)
{
  ...
  //puts_spike(current->name);
  //puts_spike("\n");
  //current->init.func(current->init.argc, current->init.argv);
  //thread_end();
