...
int test11_2_main(int argc, char *argv[])
{
  ...
  puts_spike("test11_2 started.\n");

  /* 静的領域をメッセージで送信 */
  puts_spike("test11_2 send in.\n");
  kz_send(MSGBOX_ID_MSGBOX1, 15, "static memory\n");
  puts_spike("test11_2 send out.\n");
  ...
  /* 静的領域をメッセージで受信 */
  puts_spike("test11_2 recv in.\n");
  kz_recv(MSGBOX_ID_MSGBOX2, &size, &p);
  puts_spike("test11_2 recv out.\n");
  puts_spike(p);
