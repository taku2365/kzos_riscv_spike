...
int test11_1_main(int argc, char *argv[])
{
  ...
  puts_spike("test11_1 started.\n");

  /* 静的領域をメッセージで受信 */
  puts_spike("test11_1 recv in.\n");
  kz_recv(MSGBOX_ID_MSGBOX1, &size, &p);
  puts_spike("test11_1 recv out.\n");
  puts_spike(p);
  ...
  /* 静的領域をメッセージで送信 */
  puts_spike("test11_1 send in.\n");
  kz_send(MSGBOX_ID_MSGBOX2, 15, "static memory\n");
  puts_spike("test11_1 send out.\n");
